...
ubuntu_systems() {
    ...
    add-apt-repository \
        "deb [arch=${ARCH_GO}] https://download.docker.com/linux/ubuntu \
         $(lsb_release -cs) \
         stable"
...
EOF
    ...
    # update setuptools for aarch64 to work
    ...
    HELM_SHA256SUM="HELM_SHA256SUM_${ARCH_GO}"
    ...
    PROTOC_SHA256SUM="PROTOC_SHA256SUM_${ARCH_UNDER}"
    curl -L -o "/tmp/protoc-${PROTOC_VERSION}-linux-${ARCH_UNDER}.zip" "https://github.com/google/protobuf/releases/download/v${PROTOC_VERSION}/protoc-${PROTOC_VERSION}-linux-${ARCH_UNDER}.zip"
    echo "${!PROTOC_SHA256SUM}  /tmp/protoc-${PROTOC_VERSION}-linux-${ARCH_UNDER}.zip" | sha256sum -c -
