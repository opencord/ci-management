...
def process () {
    ...
    String iam = 'vars/installKail.groovy'
    println("** ${iam}: ENTER")
    ...
    sh returnStdout: false, script: """#!/bin/bash
      ...
      kail_name="kail_\${kail_ver}_\${HOSTOS}_\${HOSTARCH}.tar.gz"
      curl -o "\${tmp_dir}/\${kail_name}" -sSL "\${download_url}/\${kail_ver}/\${kail_name}"

      (cd "\${tmp_dir}" && tar -xzf "\${kail_name}")
      test ! -d "\${bin_dir}" && install -d "\${bin_dir}"
      install "\${tmp_dir}/kail" "\${bin_dir}/"
