    ...
    stage('build'){
      steps {
        sh( script: """
          ...
          then
            EXTRA_GO_BUILD_TAGS="-tags profile" DOCKER_TAG="$branchName-profile" make docker-build 2>&1 | tee "$WORKSPACE/docker-build-profile.log"
          fi
          ...
          then
            ...
            do
              ...
              if [$buildProfiled=true]
              then
                EXTRA_GO_BUILD_TAGS="-tags profile" DOCKER_TAG="\$clean_tag-profile" make docker-build
              fi
            done
          fi
        """)
        ...
        script {
          withDockerRegistry([credentialsId: 'docker-artifact-push-credentials']) {
            sh( script:"""
              ...
              if [$buildProfiled=true]
              then
                EXTRA_GO_BUILD_TAGS="-tags profile" DOCKER_TAG="$branchName-profile" make docker-push 2>&1 | tee "$WORKSPACE/docker-push-profile.log"
              fi
              ...
              then
                ...
                do
                  ...
                  if [$buildProfiled=true]
                  then
                    EXTRA_GO_BUILD_TAGS="-tags profile" DOCKER_TAG="\$clean_tag-profile" make docker-push
                  fi
                done
              fi
            """)
