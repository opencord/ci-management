  ...
  stages {
    stage ('Initialize') {
      steps {
        ...
        // remove OLT logs from previous builds
        script {
          deployment_config.olts.each { olt ->
            sh returnStdout: false, script: """
            sshpass -p ${olt.pass} ssh -o UserKnownHostsFile=/dev/null -o StrictHostKeyChecking=no ${olt.user}@${olt.sship} rm -f /var/log/startup.log || true
            """
          }
        }
