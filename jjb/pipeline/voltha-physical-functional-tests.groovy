  ...
  stages {
    ...
    stage('Subscriber Validation and Ping Tests') {
      ...
      steps {
        ...
        if  ( ${released} ); then
            export ROBOT_MISC_ARGS="--removekeywords wuks -i released -e bbsim -e notready -d $WORKSPACE/RobotLogs -v POD_NAME:${configFileName} -v KUBERNETES_CONFIGS_DIR:$WORKSPACE/${configBaseDir}/${configKubernetesDir}"
        else
            export ROBOT_MISC_ARGS="--removekeywords wuks -i sanity -d $WORKSPACE/RobotLogs -v POD_NAME:${configFileName} -v KUBERNETES_CONFIGS_DIR:$WORKSPACE/${configBaseDir}/${configKubernetesDir}"
        fi
