    ...
    stage('Run E2E Tests') {
      steps {
        sh '''
           ...
           export MISC_ARGS=""
           if [[ ${onosVersion} == "1.13.10" ]]; then
              MISC_ARGS="-v sadis.file:${CURDIR}/../../data/onos-sadis-bbsim-new-apps.json"
           fi

           make -C $WORKSPACE/voltha/voltha-system-tests ROBOT_MISC_ARGS="$MISC_ARGS" single-kind || true
           '''
      }
    }
  }
