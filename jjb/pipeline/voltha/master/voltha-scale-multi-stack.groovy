  ...
  stages {
    ...
    stage('Deploy VOLTHA infrastructure') {
      steps {
        ...
        _TAG=etcd-port-forward kubectl -n infra port-forward --address 0.0.0.0 service/etcd 9999:2379&
        '''
      }
    }
    stage('Deploy Voltha') {
  ...
  for (int i = 1; i <= numberOfStacks.toInteger(); i++) {
    stage("Test VOLTHA stack " + i) {
      timeout(time: 15, unit: 'MINUTES') {
        ...
        _TAG=voltha-port-forward kubectl port-forward --address 0.0.0.0 -n voltha${i} svc/voltha-voltha-api 55555:55555&
