...
def execute_test(workflow, testTarget, outputDir, testSpecificHelmFlags = "") {
  ...
  stage('Run test ' + testTarget + ' on ' + workflow + ' workFlow') {
    sh """
    mkdir -p $WORKSPACE/${robotLogsDir}
    for i in \$(seq 1 ${testRuns})
    do
      export ROBOT_MISC_ARGS="${params.extraRobotArgs} -d $WORKSPACE/${robotLogsDir}/\$i -e PowerSwitch"
...
pipeline {
  ...
  stages {
    ...
    stage('Run E2E Tests') {
      steps {
        execute_test("att", makeTarget, "att")
       }
     }
  }
  post {
