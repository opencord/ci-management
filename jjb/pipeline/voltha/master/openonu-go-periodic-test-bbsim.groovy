...
def execute_test(testName, workflow, testTarget, outputDir, testSpecificHelmFlags = "") {
  ...
  stage('Run test ' + testTarget + ' on ' + workflow + ' workFlow') {
    ...
    export ROBOT_MISC_ARGS="-d ${robotLogsDir}/${testName}"
    export KVSTOREPREFIX=service/voltha/voltha_voltha
  ...
  stages {
    ...
    stage('Run MIB Upload Tests') {
      ...
      steps {
        script {
          ...
          mibUploadHelmFlags += "--set --set pon=2,onu=2,controlledActivation=only-onu "
        }
    ...
    stage('Reconcile ATT workflow') {
      steps {
        ...
        execute_test("ReconcileATT", "dt", makeReconcileTestTarget, "$WORKSPACE/ReconcileATT", reconcileHelmFlags)
      }
    }
