  ...
  stages{
    ...
    stage('Deploy Device Manager Interface Chart') {
      steps {
        script {
          sh returnStdout: false, script: """
            helm install ${params.extraHelmFlags} olt-device-manager ${params.dmiChart}
          """
        }
        println "Wait for olt-device-manager to start"
        sh """
            set +x
            devmgr=\$(kubectl get pods -l app.kubernetes.io/name=adtran-olt-device-manager --no-headers | grep "0/" | wc -l)
            while [[ \$devmgr != 0 ]]; do
              sleep 5
              devmgr=\$(kubectl get pods -l app.kubernetes.io/name=adtran-olt-device-manager --no-headers | grep "0/" | wc -l)
            done
        """
        sh """
          JENKINS_NODE_COOKIE="dontKillMe" _TAG="adtran-olt-device-manager" bash -c "while true; do kubectl port-forward --address 0.0.0.0 svc/adtran-olt-device-manager 50051:10000; done"&
          ps aux | grep port-forward
        """
      }
    }
    ...
    stage('Run Device Management Interface Tests') {
      environment {
        ...
        ROBOT_TEST_INPUT_FILE="$WORKSPACE/voltha-system-tests/tests/data/dmi-components-adtran.yaml"
      }
      steps {
        sh """
          mkdir -p $ROBOT_LOGS_DIR
          export ROBOT_MISC_ARGS="--removekeywords wuks -e notreadyDMI -i functionalDMI -d $ROBOT_LOGS_DIR -V $ROBOT_TEST_INPUT_FILE"
