  ...
  stages {
    ...
    stage('Build olt addr list') {
      steps {
        script {
          sh """
          echo "addrs:" > /tmp/robot_vars
          """
          deployment_config.olts.each { olt ->
            sh """
            echo " - ${olt.ip}:8888" >> /tmp/robot_vars
            """
          }
          sh """
          cat /tmp/robot_vars
    ...
    stage('Run E2E Tests') {
      steps {
        sh '''
           ...
           make -C $WORKSPACE/device-management ROBOT_EXTRA_ARGS="-V /tmp/robot_vars"  functional-physical-test-single || true
           '''
      }
    }
