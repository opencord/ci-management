  ...
  stages {
    ...
    stage('Kubernetes Functional Tests') {
      steps {
        sh '''
           rm -rf $WORKSPACE/RobotLogs; mkdir -p $WORKSPACE/RobotLogs
           #scale in etcd, change to Makefile command later
           source $WORKSPACE/voltha-system/bin/activate
           robot -i etcddown $WORKSPACE/voltha-system-tests/tests/functional/K8S_SystemTest.robot
           make -C $WORKSPACE/voltha-system-tests ${makeTarget} || true
           #scale out etcd
           robot -i etcdup $WORKSPACE/voltha-system-tests/tests/functional/K8S_SystemTest.robot
           '''
      }
    }
  }
