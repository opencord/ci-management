...
pipeline {
  ...
  post {
    ...
    always {
      ...
      '''
      sh '''
        kail -d adapter-open-olt > open-olt-logs.txt
        kail -d adapter-open-onu > open-onu-logs.txt
        kail -d voltha-rw-core > voltha-rw-core-logs.txt
        kail -d voltha-ofagent > voltha-ofagent-logs.txt
        kail -d bbsim > bbsim-logs.txt
      '''
      sh '''
