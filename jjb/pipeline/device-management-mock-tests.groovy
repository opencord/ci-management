...
pipeline {
  ...
  stages {
    ...
    stage('Build Redfish Importer Image') {
      steps {
        sh """
           make -C $WORKSPACE/device-management/\$1 DOCKER_REPOSITORY=voltha/ DOCKER_TAG=citest docker-build-importer
           """
      }
    }
    ...
    stage('Build demo_test Image') {
      steps {
        sh """
           make -C $WORKSPACE/device-management/\$1/demo_test DOCKER_REPOSITORY=voltha/ DOCKER_TAG=citest docker-build
           """
      }
    }
    ...
    stage('Build mock-redfish-server  Image') {
      steps {
        sh """
           make -C $WORKSPACE/device-management/\$1/mock-redfish-server DOCKER_REPOSITORY=voltha/ DOCKER_TAG=citest docker-build
           """
      }
    }
