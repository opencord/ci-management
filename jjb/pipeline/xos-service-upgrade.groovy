    ...
    stage('Test Pre-Upgrade') {
      steps {
        sh """
           ...
           for i in \$SERVICES; do bash -c "robot -v SETUP_FLAG:Setup -i create -d Log -T -e ProgranServiceInstance -v TESTLIBRARY:${gerritProject}_library.robot \$i\$testname"; sleep 2; done || true
    ...
    stage('Test Post-Upgrade') {
      steps {
        sh """
           ...
           echo \$SERVICES
           cd $WORKSPACE/cord/test/cord-tester/src/test/cord-api/Tests
           for i in \$SERVICES; do bash -c "robot -v SETUP_FLAG:Setup -i get -d Log -T -e ProgranServiceInstance -v TESTLIBRARY:${gerritProject}_library.robot \$i\$testname"; sleep 2; done || true
