...
pipeline {
  ...
  stages {
    ...
    stage ('Build voltha and onos') {
      steps {
        sh 'vagrant ssh -c "cd /cord/incubator/voltha && source env.sh && make fetch-jenkins && make jenkins && make onos" voltha' }
        }

    stage ('Start Provisioning Test') {
      steps {
        println 'Start Provisioning Test'
        sh 'vagrant ssh -c "cd /cord/incubator/voltha/tests && pwd" voltha' }
      }
    }
}
